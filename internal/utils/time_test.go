...
const (
	...
	previousTime int64 = 1628042610000000000 // GMT: Wednesday, August 4, 2021 2:03:30 AM
)

func TestTime_GetMinimumLastHealthPacketTime_ReturnsExpectedTimeWhenZeroIsPassed(t *testing.T) {
	assert.Equal(t, int64(previousTime), GetMinimumLastHealthPacketTime(time.Unix(setTime, 0), 0))
}

func TestTime_GetMinimumLastHealthPacketTime_ReturnsExpectedTimeWhenDelayIsPassed(t *testing.T) {
	assert.Equal(t, int64(previousTime), GetMinimumLastHealthPacketTime(time.Unix(setTime, 0), 2))
}
